...
class HomeViewController: UIViewController {
    ...
    private var photoArrayFromApiFetch = [ImagesModel]()
    ...
    func fetchedPhotos() {
        NetworkService.shared.createArrayFromAPI(offset: photoArrayFromApiFetch.count) { [weak self] result, error in
            if let error = error {
                print("Error: \(error)")
                return
            }
...
extension HomeViewController {
    ...
    static func generateLayout() -> UICollectionViewLayout {
        ...
        // 1
        let mainWithPairGroup = NSCollectionLayoutGroup.horizontal(layoutSize: NSCollectionLayoutSize(widthDimension: .fractionalWidth(1), heightDimension: .fractionalWidth(4/9)), subitems: [mainItem, trailingGroup])
...
extension HomeViewController: UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        detailViewController.imageArray = self.photoArrayFromApiFetch
